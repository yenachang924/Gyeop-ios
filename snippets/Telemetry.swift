...
enum Log {
    ...
    static let nearby = Logger(subsystem: subsystem, category: "nearby")
...
/// defer { span.end() }
/// ```
enum ExchangePhase: StaticString {
    /// 광고 시작 → 피어 발견
    ...
    func begin() -> SignpostSpan {
        let id = Signpost.nearby.makeSignpostID()
        let state = Signpost.nearby.beginInterval(rawValue, id: id)
        return SignpostSpan(
...
struct SignpostSpan {
    ...
    func end(success: Bool = true) {
        ...
        Log.nearby.info("\(name, privacy: .public) \(success ? "ok" : "fail", privacy: .public) \(ms, format: .fixed(precision: 1))ms")
    }
}
...
actor MetricCounter {
    static let shared = MetricCounter()

    private var counts: [String: Int] = [:]

    func increment(_ key: String, by n: Int = 1) {
...
enum Metric {
    static let exchangeAttempt = "exchange.attempt"
    static let exchangeSuccess = "exchange.success"
    static let exchangeFailure = "exchange.failure"
    static let exchangeRetry = "exchange.retry"
    static let exchangeDuplicate = "exchange.duplicate"   // 멱등성 검증용 — 아티클 #2

    static let signalCreated = "signal.created"
    static let signalJoined = "signal.joined"
    static let signalFulfilled = "signal.fulfilled"        // 정원 충족
    static let signalExpired = "signal.expired"
    static let pushReceived = "push.received"

    static let syncQueued = "sync.queued"
    static let syncUploaded = "sync.uploaded"
    static let syncConflict = "sync.conflict"
}
...
enum GyeopID {
    static let slotSeconds: TimeInterval = 300  // 5분

    static func make(runnerA: String, runnerB: String, serverCorrectedDate: Date) -> String {
        let pair = [runnerA, runnerB].sorted().joined(separator: "|")
        let slot = Int(serverCorrectedDate.timeIntervalSince1970 / slotSeconds)
        return "\(pair)|\(slot)".sha256()
    }
}

private extension String {
    func sha256() -> String {
        // CryptoKit import 후 구현 — 자리 표시
        // import CryptoKit
        // return SHA256.hash(data: Data(utf8)).map { String(format: "%02x", $0) }.joined()
        fatalError("CryptoKit 구현 필요")
    }
}
